"""
cogs/academia.py
Sistema de seleção de cargos acadêmicos inspirado no discord-math/bot.
— Botão "Gerenciar cargos" persistente
— Dropdown de nível acadêmico (Profissional, Graduação+, Mestrado, Doutorado, Pós-doutorado)
— Formulário filosófico para Mestrado e acima
— Cargo temporário "Pendente" enquanto aguarda aprovação
— Canal de revisão com botões Aprovar / Rejeitar
"""
...
log = logging.getLogger("sophosbot.academia")

# Níveis que requerem análise manual (formulário)
...
class PhilosophyApplicationModal(discord.ui.Modal):
    area = discord.ui.TextInput(
        ...
        placeholder="Ex: Filosofia da Mente, Metaética, Teologia Sistemática...",
        max_length=600
    )
    formacao = discord.ui.TextInput(
        label="Qual é sua formação acadêmica?",
        style=discord.TextStyle.paragraph,
        placeholder="Você pode omitir detalhes que te identifiquem, se preferir.",
        max_length=600
    )
    obras = discord.ui.TextInput(
        label="Quais obras/autores você tem estudado recentemente?",
        style=discord.TextStyle.paragraph,
        placeholder="Diga o que achou interessante!",
        max_length=600
    )
...
class AcademicLevelSelect(discord.ui.Select):
    def __init__(self, storage: Storage, guild_id: int):
        ...
        # Busca os cargos configurados para mostrar o que existe
        options = []
        for key, label in LEVEL_LABELS.items():
            role_id = storage.guild_get(guild_id, "academia_config", f"role_{key}")
    ...
    async def callback(self, inter: discord.Interaction):
        level_key = self.values[0]
        cog = inter.client.cogs.get("Academia")
        if not cog:
            return

        if level_key in REQUIRES_REVIEW:
            # Abre o formulário para análise
            await inter.response.send_modal(PhilosophyApplicationModal(level_key))
        else:
            # Atribuição direta
            await inter.response.defer(ephemeral=True)
            await cog.assign_direct_role(inter, level_key)
...
class ManageRolesView(discord.ui.View):
    ...
    async def manage_roles(self, inter: discord.Interaction, button: discord.ui.Button):
        ...
        # Verifica se há ao menos um cargo configurado
        has_any = any(
            storage.guild_get(guild_id, "academia_config", f"role_{k}")
            for k in LEVEL_LABELS
        )
        if not has_any:
            return await inter.response.send_message(
                embed=embed_error("Não configurado", "Os cargos acadêmicos ainda não foram configurados. Peça a um admin para usar `/academia setup`."),
                ephemeral=True
            )

        view = discord.ui.View(timeout=60)
        view.add_item(AcademicLevelSelect(storage, guild_id))

        emb = discord.Embed(
            title=f"{E['trophy']} Nível Acadêmico",
            description=(
                ...
                f"{E['n3']} **Mestrado** — requer formulário\n"
                f"{E['n4']} **Doutorado** — requer formulário\n"
                f"{E['n5']} **Pós-doutorado** — requer formulário"
            ),
            color=PHILO_COLOR
        )
...
class Academia(commands.Cog):
    def __init__(self, bot):
        ...
        # Registrar views de revisão pendentes
        self._restore_review_views()

    def _restore_review_views(self):
        """Restaura views de revisão pendentes após reinício."""
        pending = self.storage.load("academia_pending")
    ...
    def _cfg(self, guild_id, key, default=None):
        return self.storage.guild_get(guild_id, "academia_config", key) or default

    def _set(self, guild_id, key, value):
        self.storage.guild_set(guild_id, "academia_config", key, value)

    def _remove_all_academic_roles(self, member: discord.Member):
        """Remove todos os cargos acadêmicos do membro."""
        role_ids = set()
        ...
        # Cargo pendente também
        pending_rid = self._cfg(guild_id, "role_pending")
        if pending_rid:
            role_ids.add(int(pending_rid))
    ...
    async def assign_direct_role(self, inter: discord.Interaction, level_key: str):
        ...
        if not role_id:
            return await inter.followup.send(embed=embed_error("Cargo não configurado", f"O cargo para **{LEVEL_LABELS[level_key]}** ainda não foi configurado."), ephemeral=True)
        ...
        member = inter.user
        # Remove outros cargos acadêmicos
        to_remove = self._remove_all_academic_roles(member)
        if to_remove:
        ...
        await inter.followup.send(embed=embed_success("Cargo atribuído!", f"{E['arrow_blue']} Você recebeu o cargo {role.mention}."), ephemeral=True)

    async def handle_application(self, inter: discord.Interaction, level_key: str, answers: dict):
        """Processa a candidatura de nível avançado."""
        guild = inter.guild
        await inter.response.defer(ephemeral=True)

        # Atribuir cargo pendente
        ...
        await inter.followup.send(
            embed=embed_info("Candidatura enviada!", f"{E['loading']} Sua candidatura para **{LEVEL_LABELS[level_key]}** foi enviada para análise.\n\nVocê recebeu o cargo {E['dash']} **Pendente** enquanto aguarda."),
            ephemeral=True
        )
    ...
    async def approve_application(self, inter: discord.Interaction, applicant_id: int, level_key: str):
        ...
        if not data:
            return await inter.followup.send(embed=embed_error("Não encontrado", "Candidatura não encontrada."), ephemeral=True)
        ...
        # Remove cargos acadêmicos anteriores + pendente
        to_remove = self._remove_all_academic_roles(member)
        if to_remove:
        ...
        await inter.message.edit(embed=emb_edit, view=None)
    ...
    async def reject_application(self, inter: discord.Interaction, applicant_id: int, level_key: str):
        ...
        if member:
            # Remove cargo pendente
            pending_rid = self._cfg(guild.id, "role_pending")
            if pending_rid:
                pending_role = guild.get_role(int(pending_rid))
                if pending_role and pending_role in member.roles:
                    await member.remove_roles(pending_role, reason="Candidatura rejeitada")
            # Notifica
            try:
                emb_dm = discord.Embed(
        ...
        emb_edit = discord.Embed(
            ...
            description=f"{E['arrow_white']} **Usuário:** {member.mention if member else f'ID {applicant_id}'}\n{E['pin']} **Rejeitado por:** {inter.user.mention}",
            color=0xED4245
        )
        await inter.message.edit(embed=emb_edit, view=None)
    ...
    @app_commands.describe(canal="Canal onde enviar o painel", titulo="Título da embed", descricao="Descrição")
    @app_commands.default_permissions(administrator=True)
    async def academia_painel(self, inter: discord.Interaction,
        ...
        emb.set_footer(text="SophosBot • Academia")
        await canal.send(embed=emb, view=ManageRolesView())
        await inter.response.send_message(embed=embed_success("Painel enviado!", f"Painel acadêmico enviado em {canal.mention}."), ephemeral=True)

    @academia_group.command(name="pendentes", description="Lista candidaturas pendentes")
    ...
    async def academia_pendentes(self, inter: discord.Interaction):
        ...
        if not guild_pending:
            return await inter.response.send_message(embed=embed_info("Sem pendências", "Não há candidaturas aguardando análise."), ephemeral=True)
    ...
    async def academia_remove(self, inter: discord.Interaction, membro: discord.Member):
        await inter.response.defer(ephemeral=True)
        to_remove = self._remove_all_academic_roles(membro)
        if to_remove:
            await membro.remove_roles(*to_remove, reason=f"Remoção por {inter.user}")
        await inter.followup.send(embed=embed_success("Cargos removidos!", f"Cargos acadêmicos de {membro.mention} removidos."), ephemeral=True)
